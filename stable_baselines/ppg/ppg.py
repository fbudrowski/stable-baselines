...
from stable_baselines.common.policies import ActorCriticPolicy, RecurrentActorCriticPolicy, nature_cnn, mlp_extractor
from stable_baselines.common.schedules import get_schedule_fn
from stable_baselines.common.tf_layers import linear
from stable_baselines.common.tf_util import total_episode_reward_logger
from stable_baselines.common.math_util import safe_mean

class PPGFeedForwardPolicy(ActorCriticPolicy):
    """
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, layers=None, net_arch=None,
                 separate_vf_layers=[64, 64],
                 act_fun=tf.tanh, cnn_extractor=nature_cnn, feature_extraction="cnn", **kwargs):
        super(PPGFeedForwardPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=reuse,
        ...
        with tf.variable_scope("model", reuse=reuse):
            if feature_extraction == "cnn":
                ...
                _, vf_latent = mlp_extractor(tf.layers.flatten(self.processed_obs), [dict(vf=separate_vf_layers, pi=[64])],
                                             act_fun, type="separate")


            self._joined_value_fn = linear(joint_vf_latent, 'joint_vf', 1)
            self._joined_value_flat = self._joined_value_fn[:, 0]
    ...
    @property
    def joined_value_flat(self):
        return self._joined_value_flat

    def step(self, obs, state=None, mask=None, deterministic=False):
        if deterministic:
            action, value, neglogp, value_joint = self.sess.run([self.deterministic_action, self.value_flat, self.neglogp, self.joined_value_flat],
                                                   {self.obs_ph: obs})
        else:
            action, value, neglogp, value_joint = self.sess.run([self.action, self.value_flat, self.neglogp, self.joined_value_flat],
                                                   {self.obs_ph: obs})
        return action, value, self.initial_state, neglogp, value_joint
    ...
    def joined_value(self, obs, state=None, mask=None):
        return self.sess.run(self.joined_value_flat, {self.obs_ph: obs})

class PPGCnnPolicy(PPGFeedForwardPolicy):
    """
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, **_kwargs):
        super(PPGCnnPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse,
                                        feature_extraction="cnn", **_kwargs)

class PPGMlpPolicy(PPGFeedForwardPolicy):
    """
    Policy object that implements actor critic, using a MLP (2 layers of 64)
    """

    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, **_kwargs):
        super(PPGMlpPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse,
                                        feature_extraction="mlp", **_kwargs)



class PPG(ActorCriticRLModel):
    """
    Phasic Policy Gradient algorithm (GPU version).
    ...
    def setup_model(self):
        with SetVerbosity(self.verbose):
            ...
            assert issubclass(self.policy, PPGFeedForwardPolicy), "Error: the input policy for the PPG model must be " \
                                                               "an instance of a special feedforward policy."
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    grads_aux = list(zip(grads_aux, self.params))
    ...
    def _train_step(self, learning_rate, cliprange, obs, returns, masks, actions, values, neglogpacs, update,
        ...
        # print("PG loss", policy_loss, "Value loss", value_loss, "KL", approxkl)
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for phase in range(1, n_phases + 1):
                ...
                for aux_phase in range(1, self.auxiliary_phases + 1):
                    ...
                    assert (self.n_batch * self.policy_phases) % self.nminibatches == 0, ("The number of minibatches (`nminibatches`) "
                                                                   "is not a factor of the total number of samples "
                                                                   "collected per rollout (`n_batch`), "
                                                                   "some samples won't be used."
                                                                   )
...
class Runner(AbstractEnvRunner):
    ...
    def _run(self):
        ...
        mb_obs, mb_rewards, mb_actions, mb_values, mb_values_joint, mb_dones, mb_neglogpacs = [], [], [], [], [], [], []
        mb_states = self.states
        ep_infos = []
        for _ in range(self.n_steps):
            actions, values, self.states, neglogpacs, values_joint = self.model.step(self.obs, self.states, self.dones)
            mb_obs.append(self.obs.copy())
            mb_actions.append(actions)
            mb_values.append(values)
            mb_values_joint.append(values_joint)
        ...
        mb_values_joint = np.asarray(mb_values_joint, dtype=np.float32)
